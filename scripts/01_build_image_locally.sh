  ...
  then
    ...
    then
      ...
      then
        ...
        echo "${JSON_APP}" | jq '.'
        echo "to try your container image locally :  ${DOCKER_BIN} run --rm -p 9999:9999 --env-file .env  --name ${APP_NAME_SNAKE} ${CONTAINER_REGISTRY_ID}/${APP_NAME_SNAKE}"
      ...
      echo "## 💥💥 ERROR: please upgrade version number in pkg/version/version.go file if you really want to rebuild !"

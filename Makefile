...
openapi-codegen: dependencies-openapi
	oapi-codegen --old-config-style -generate types -o pkg/trees/trees_types.gen.go -package trees api/trees.yaml
	oapi-codegen --old-config-style -templates templates_oapi-codegen -generate server -o pkg/trees/trees_server.gen.go -package trees api/trees.yaml
...
xo-codegen: dependencies-xo
	xo schema schema -s public --go-pkg=trees --src templates_xo -o models ${DB_DRIVER}://${DB_USER}:=@${DB_HOST}/${DB_NAME}
